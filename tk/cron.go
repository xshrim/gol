...
func NewCron() *Crontab {
	return newCron(time.Minute)
}

// new creates new crontab, arg provided for testing purpose
func newCron(t time.Duration) *Crontab {
	c := &Crontab{
...
func (c *Crontab) ShutdownCron() {
	c.ticker.Stop()
}

// Clear all jobs from cron table
func (c *Crontab) ClearCron() {
	c.Lock()
	c.jobs = []*job{}
	c.Unlock()
}
...
func (j *job) tickCron(t tick) bool {
	j.RLock()
	defer j.RUnlock()
...
func getCronTick(t time.Time) tick {
	return tick{
		min:       t.Minute(),
		hour:      t.Hour(),
		day:       t.Day(),
