...
/*
FuzzyFind looks up pattern in data and returns matches
...
*/
func FuzzyFind(pattern string, data []string) FuzzyMatches {
	return FuzzyFindFrom(pattern, stringSource(data))
}

/*
FuzzyFindNoSort is an alternative Find implementation that does not sort
the results in the end.
*/
func FuzzyFindNoSort(pattern string, data []string) FuzzyMatches {
	return FuzzyFindFromNoSort(pattern, stringSource(data))
}

/*
FuzzyFindFrom is an alternative implementation of Find using a Source
instead of a list of strings.
*/
func FuzzyFindFrom(pattern string, data FuzzySource) FuzzyMatches {
	matches := FuzzyFindFromNoSort(pattern, data)
	sort.Stable(matches)
	return matches
}

/*
FuzzyFindFromNoSort is an alternative FindFrom implementation that does
not sort results in the end.
*/
func FuzzyFindFromNoSort(pattern string, data FuzzySource) FuzzyMatches {
	if len(pattern) == 0 {
		return nil
	}
	runes := []rune(pattern)
			...
			if equalFold(nextp, nextc) || nextc == 0 {
				if matchedIndex > -1 {
					if len(match.MatchedIndexes) == 0 {
						...
						bestScore += fuzzymax(penalty, maxUnmatchedLeadingCharPenalty)
					}
